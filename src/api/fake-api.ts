...
export class FakeApi implements ApiInterface{
  ...
  async getProductsByRange(start: number, take: number, filter: string | undefined, categoryId: string | undefined) {
    // get all
    const allProducts : TProduct[] = (await fetch(Config.ProductListURL).then(res => res.json())) || [];
